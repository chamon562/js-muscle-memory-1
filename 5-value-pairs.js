...
const crypto1 = { coin: "Bitcoin", currentPrice: "$" + 56000 }
const crypto2 = { coin: "Doge", currentPrice: "$" + 0.7530 }

const getCryptoValue = (crypt1, crypt2, key) => {
  let newArray = [];
  let mainObj = [crypt1, crypt2];
  for(key1 in mainObj){
    if(key === "coin"){
      newArray.push(mainObj[key1].coin);
    } else if(key === "currentPrice"){
      newArray.push(mainObj[key1].currentPrice);
    }
  }
  return newArray;
}

console.log(getCryptoValue(crypto1, crypto2, "currentPrice"))
console.log(getCryptoValue(crypto1, crypto2, "coin"))